...
def get_metrics():
    ...
    """
    avg_latency = (
        float(sum(LATENCY_HISTOGRAM) / len(LATENCY_HISTOGRAM))
        if LATENCY_HISTOGRAM
        else 0.0
    )
    p95_latency = (
        float(np.percentile(LATENCY_HISTOGRAM, 95)) if LATENCY_HISTOGRAM else 0.0
    )
    ...
    return {
        ...
        "latency": {
            ...
            "sla_violation_rate": round(
                sum(1 for x in LATENCY_HISTOGRAM if x > 50.0)
                / max(len(LATENCY_HISTOGRAM), 1)
                * 100.0,
                2,
            ),
        },
