...
try:
    ...
    HAS_NEO4J = False
    logger.warning(
        "Neo4j python package not installed. Running Neo4jClient in simulator mode."
    )
...
class Neo4jClient:
    ...
    def fetch_transaction_subgraph(
        self, account_id: str, hops: int = 2
    ) -> List[Dict[str, Any]]:
        """
        Retrieve localized transaction subgraph around an account ID for real-time model scoring.
        """
        logger.info(
            f"Querying {hops}-hop subgraph paths for Account ID: {account_id}..."
        )
    ...
    def write_transaction_node(self, tx_data: Dict[str, Any]) -> None:
        ...
        logger.debug(
            f"Transaction {tx_data.get('txId')} written to Neo4j successfully."
        )
