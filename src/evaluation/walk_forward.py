...
class WalkForwardValidator:
    ...
    def split(self, data: pd.DataFrame, time_col: str = "step") -> Generator[Tuple[pd.DataFrame, pd.DataFrame], None, None]:
        """
        ...
        """
        logger.info(f"Setting up {self.n_splits}-fold temporal walk-forward split on column '{time_col}'...")
        ...
        if len(unique_times) < self.n_splits + 1:
            ...
            logger.warning("Fewer timesteps than requested splits. Splitting based on index ranges.")
        ...
        for fold in range(1, self.n_splits + 1):
            ...
            val_boundary = unique_times[min((fold + 1) * fold_size, len(unique_times) - 1)]
            
            train_df = data[data[time_col] < train_boundary]
            val_df = data[(data[time_col] >= train_boundary + self.gap_steps) & (data[time_col] <= val_boundary)]
            
            logger.info(f"Fold {fold}: Train steps < {train_boundary}, Val steps [{train_boundary + self.gap_steps} to {val_boundary}]")
