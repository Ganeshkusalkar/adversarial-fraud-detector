...
class FraudEvaluationEngine:
    def __init__(self, config: dict):
        ...
        self.decision_threshold = config.get("training", {}).get("decision_threshold", 0.38)
    ...
    def calculate_adversarial_robustness(self, hardened_model: torch.nn.Module, generator: torch.nn.Module, n_attacks: int = 1000, device: str = "cpu") -> float:
        """
        ...
        logger.info(f"Simulating {n_attacks} automated adversarial mutation stress tests against the GNN...")
        ...
        with torch.no_grad():
            for _ in range(n_attacks):
                ...
                padded_features = torch.cat([fake_txn[:, 0, :], torch.zeros((1, pad_size), device=device)], dim=-1)
        ...
        logger.info(f"Adversarial Robustness Score: {robustness_score:.3f} ({caught_attacks}/{n_attacks} attacks intercepted)")
    ...
    def find_optimal_threshold(self, y_true: np.ndarray, y_prob: np.ndarray) -> float:
        ...
        for idx in top_10_idx:
            logger.info(f"  Threshold: {thresholds[idx]:.4f} | Precision: {precision[idx]:.4f} | Recall: {recall[idx]:.4f} | F1: {f1_scores[idx]:.4f}")
        ...
        if len(valid_recall_idx) > 0:
            best_90_idx = valid_recall_idx[-1] # thresholds are increasing, recall is decreasing
            logger.info(f"Threshold for >= 90% Recall: {thresholds[best_90_idx]:.4f} (Recall: {recall[best_90_idx]:.4f}, Precision: {precision[best_90_idx]:.4f})")
            
        return float(optimal_threshold)

    def find_threshold_with_constraints(self, y_true: np.ndarray, y_prob: np.ndarray, min_recall: float = 0.85, min_precision: float = 0.40) -> float:
        """
        ...
        while current_min_precision >= 0.0:
            valid_idx = np.where((recall >= min_recall) & (precision >= current_min_precision))[0]
            if len(valid_idx) > 0:
                valid_f1s = f1_scores[valid_idx]
                best_valid_idx = valid_idx[np.argmax(valid_f1s)]
                best_thresh = float(thresholds[best_valid_idx])
                logger.info(f"Found threshold {best_thresh:.4f} satisfying Recall >= {min_recall} and Precision >= {current_min_precision:.2f}")
                return best_thresh
            else:
                logger.warning(f"No threshold satisfies Recall >= {min_recall} and Precision >= {current_min_precision:.2f}. Relaxing precision by 0.05...")
                current_min_precision -= 0.05
                
        logger.warning("Failed to find any constrained threshold. Falling back to global optimal F1 threshold.")
