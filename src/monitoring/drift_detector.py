...
class DriftDetector:
    ...
    def check_drift(
        self, live_batch: np.ndarray, feature_names: List[str]
    ) -> Dict[str, float]:
        ...
        for i, feat_name in enumerate(feature_names):
            if feat_name in self.reference_stats:
                expected_dist = np.array(
                    self.reference_stats[feat_name]["distribution"]
                )
                ...
                if psi > 0.25:
                    logger.critical(
                        f"Severe Data Drift Detected on {feat_name}! PSI: {psi:.4f}"
                    )
                elif psi > 0.1:
                    logger.warning(
                        f"Moderate Data Drift on {feat_name}. PSI: {psi:.4f}"
                    )
