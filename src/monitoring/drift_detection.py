...
def calculate_psi(expected, actual, buckets=10):
    ...
    psi_value = np.sum((expected_fractions - actual_fractions) * np.log(expected_fractions / actual_fractions))
    return psi_value
...
if __name__ == "__main__":
    ...
    for feat, stats in results['features'].items():
        print(f"Feature: {feat}, PSI: {stats['psi_value']:.4f}, Drift: {stats['is_drifting']}")
