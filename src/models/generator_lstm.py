...
class FraudTransactionGenerator(nn.Module):
    def __init__(self, noise_dim: int = 64, hidden_dim: int = 128, sequence_length: int = 10, feature_dim: int = 345, num_layers: int = 2):
        """
        ...
        """
        super().__init__()
        logger.info(f"Initializing FraudTransactionGenerator: Noise ({noise_dim}) -> Sequence Features ({feature_dim})")
    ...
    def sample_noise(self, batch_size: int, device: str = "cpu") -> torch.Tensor:
        ...
        """
        return torch.randn(batch_size, self.sequence_length, self.noise_dim, device=device)