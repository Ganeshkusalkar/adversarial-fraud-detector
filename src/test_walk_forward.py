...
def run_walk_forward_test():
    ...
    for fold_idx, (train_df, val_df) in enumerate(validator.split(processed_df, time_col="TransactionDT"), 1):
        ...
        train_graph.x = torch.tensor(scaler.fit_transform(train_graph.x.numpy()), dtype=torch.float32)
        val_graph.x = torch.tensor(scaler.transform(val_graph.x.numpy()), dtype=torch.float32)
    ...
    if avg_auc >= 0.85:
        logger.info("[PASS] STATUS: PASS - Model generalizes well to future time periods.")
    else:
