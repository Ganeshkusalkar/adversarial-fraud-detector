...
st.set_page_config(
    ...
    menu_items={"About": "GraphSAGE + LSTM GAN adversarial fraud detection system."},
)
...
with st.sidebar:
    ...
    avg_fraud_value = st.number_input(
        "Avg Fraud Transaction (₹)", value=12_000, step=1_000
    )
    analyst_hourly_cost = st.number_input(
        "Analyst Review Cost (₹/hr)", value=500, step=50
    )
...
with tab1:
    ...
    with mid_col:
        st.markdown(
            '<div class="section-header">🎯 Fraud Risk Gauge</div>',
            unsafe_allow_html=True,
        )
    ...
    with right_col:
        st.markdown(
            '<div class="section-header">🕸️ Graph Network Topology</div>',
            unsafe_allow_html=True,
        )
    ...
    with impact_col:
        st.markdown(
            '<div class="section-header">💰 Business Impact Breakdown</div>',
            unsafe_allow_html=True,
        )
    ...
    with training_col:
        st.markdown(
            '<div class="section-header">📈 Adversarial Training Convergence</div>',
            unsafe_allow_html=True,
        )
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown(
        '<div class="section-header">🧠 SHAP Explainability (Why was this flagged?)</div>',
        unsafe_allow_html=True,
    )
    ...
    with baseline_col:
        st.markdown(
            '<div class="section-header">📊 GNN vs XGBoost Baseline</div>',
            unsafe_allow_html=True,
        )
...
with tab2:
    st.markdown(
        '<div class="section-header">📊 Real-Time A/B Testing Telemetry</div>',
        unsafe_allow_html=True,
    )
    ab_kpi_col1, ab_kpi_col2, ab_kpi_col3 = st.columns(3)
    ab_kpi_a = ab_kpi_col1.empty()
    ab_kpi_b = ab_kpi_col2.empty()
    ab_kpi_z = ab_kpi_col3.empty()

    st.markdown("---")
    ab_chart_col1, ab_chart_col2 = st.columns(2)
    ab_chart_lat = ab_chart_col1.empty()
    ab_chart_rate = ab_chart_col2.empty()

with tab3:
    st.markdown(
        '<div class="section-header">💰 Interactive Business ROI Sandbox</div>',
        unsafe_allow_html=True,
    )
    roi_param_col, roi_chart_col = st.columns([1.5, 2.5])

# -----------------------------------------------------------------------
# Simulation state
# -----------------------------------------------------------------------
API_URL = "http://localhost:8000/api/v1/predict"
API_AB_URL = "http://localhost:8000/api/v1/predict_ab"
API_STATUS_URL = "http://localhost:8000/api/v1/ab_status"
...
def make_network_graph(is_fraud: bool, step: int) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=edge_y,
            mode="lines",
            line=dict(color="#1e2a45", width=1.5),
            hoverinfo="none",
        )
    )
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                size=sizes, color=colors, line=dict(color="#0f172a", width=1.5)
            ),
            text=[
                (
                    f"C{i+1}"
                    if i != target_node
                    else ("⚠ FRAUD" if is_fraud else f"C{i+1}")
                )
...
def make_impact_chart(
    caught: int, fp: int, missed: int, avg_val: int, analyst_cost: int
) -> go.Figure:
...
def make_shap_chart(shap_data: dict) -> go.Figure:
    if not shap_data:
        ...
        fig.add_annotation(
            text="Waiting for flagged transaction...",
            showarrow=False,
    ...
    fig.update_layout(
        **DARK_LAYOUT,
        height=260,
        title=dict(
            text="Top Contributing Features to Fraud Score",
...
def make_health_chart(drift_detected: bool, psi_value: float) -> go.Figure:
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            value=psi_value,
            title={
                "text": f"Population Stability Index (PSI)<br><br><span style='color:{status_color};font-size:24px'><b>{status_text}</b></span>",
...
def make_baseline_chart() -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=models, y=auc, name="ROC-AUC", marker_color="#38bdf8"))
    fig.add_trace(
        go.Bar(x=models, y=recall, name="Recall@99% Precision", marker_color="#a78bfa")
    )
...
def run_local_z_test(n_a: int, x_a: int, n_b: int, x_b: int) -> dict:
    """Two-proportion Z-test for A/B testing conversion difference significance."""
    if n_a < 5 or n_b < 5:
        return {
            "significant": False,
            "p_value": 1.0,
            "z_score": 0.0,
            "message": "Collecting A/B split telemetry...",
        }
    p_a = x_a / n_a
    p_b = x_b / n_b
    ...
    if p_pooled <= 0.0 or p_pooled >= 1.0:
        return {
            ...
            "p_value": 1.0,
            "z_score": 0.0,
            "message": "Zero variance in groups.",
        }
...
def make_ab_comparison_charts(
    ...
    """Generates average latency and fraud rate comparison bar charts for Group A and B."""
    ...
    fig_lat = go.Figure(
        go.Bar(
            x=["GraphSAGE+GAN (Group A)", "XGBoost Baseline (Group B)"],
            y=[avg_a, avg_b],
            marker_color=["#38bdf8", "#a78bfa"],
            text=[f"{avg_a:.2f} ms", f"{avg_b:.2f} ms"],
            textposition="outside",
        )
    )
    fig_lat.update_layout(
        **DARK_LAYOUT,
        height=220,
        title="Average Serving Latency (ms)",
        yaxis=dict(gridcolor="#1e2a45"),
    )

    rate_a = (fraud_a / requests_a * 100.0) if requests_a > 0 else 0.0
    rate_b = (fraud_b / requests_b * 100.0) if requests_b > 0 else 0.0
    ...
    fig_rate.update_layout(
        **DARK_LAYOUT,
        height=220,
        title="Fraud Capture Rate (%)",
        yaxis=dict(gridcolor="#1e2a45"),
    )

    return fig_lat, fig_rate
...
# -----------------------------------------------------------------------
# Tab 3 — Static ROI Sandbox Render
# -----------------------------------------------------------------------
with roi_param_col:
    st.markdown("##### ⚙️ Sandbox Parameters")
    monthly_volume = st.number_input(
        "Monthly Transactions Volume", value=500_000, step=50_000
    )
    avg_tx_value = st.number_input(
        "Average Transaction Amount (₹)", value=12_000, step=1000
    )
    true_fraud_rate = st.slider(
        "True Fraud Rate (%)", min_value=0.1, max_value=5.0, value=1.5, step=0.1
    )

    st.markdown("##### 🎯 Detection Capabilities")
    xgb_rec = st.slider(
        "XGBoost Baseline Recall (%)",
        min_value=10.0,
        max_value=90.0,
        value=22.1,
        step=0.5,
    ...
    sage_rec = st.slider(
        "GraphSAGE+GAN Recall (%)", min_value=10.0, max_value=99.0, value=78.0, step=0.5
    )

    st.markdown("##### 👥 Operations & Analysts")
    hourly_rate = st.number_input("Analyst Review Cost (₹/hour)", value=500, step=50)
    mins_per_tx = st.number_input(
        "Time per Flagged Transaction (mins)", value=10, step=1
    )

with roi_chart_col:
    # Calculations
    total_fraud_value = monthly_volume * (true_fraud_rate / 100.0) * avg_tx_value
    xgb_saved = total_fraud_value * (xgb_rec / 100.0)
    sage_saved = total_fraud_value * (sage_rec / 100.0)
    incremental_saved = sage_saved - xgb_saved

    # Analyst workload cost estimation (assuming 1% flagged rate for operations)
    flagged_a = monthly_volume * 0.012  # GraphSAGE flags ~1.2%
    flagged_b = monthly_volume * 0.005  # XGBoost flags ~0.5%
    analyst_hours_a = flagged_a * (mins_per_tx / 60.0)
    analyst_hours_b = flagged_b * (mins_per_tx / 60.0)
    op_cost_a = analyst_hours_a * hourly_rate
    op_cost_b = analyst_hours_b * hourly_rate

    net_savings_monthly = incremental_saved - (op_cost_a - op_cost_b)

    st.markdown("##### 📊 Financial Analysis & Projections")
    m_col1, m_col2 = st.columns(2)
    m_col1.metric("GraphSAGE Monthly Fraud Stopped", f"₹ {sage_saved/10000000:.2f} Cr")
    m_col2.metric(
        "Net Monthly Business ROI Uplift",
        f"₹ {net_savings_monthly/10000000:.2f} Cr",
        help="Incremental fraud stopped minus operational review overhead differences.",
    )

    months = list(range(1, 13))
    proj_xgb = [xgb_saved * m / 10000000 for m in months]
    proj_sage = [(sage_saved - op_cost_a) * m / 10000000 for m in months]
    ...
    fig_roi.update_layout(
        **DARK_LAYOUT,
        height=280,
        title="Cumulative Business Savings Projection (₹ Cr)",
        xaxis=dict(title="Month", tickmode="linear", tick0=1, dtick=1),
        yaxis=dict(title="Savings (₹ Cr)", gridcolor="#1e2a45"),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
    )
    st.plotly_chart(fig_roi, use_container_width=True)

    st.success(
        f"💡 Upgrading from XGBoost to GraphSAGE+GAN increases fraud capture from "
        f"**₹ {xgb_saved/10000000:.2f} Cr** to **₹ {sage_saved/10000000:.2f} Cr**. "
        f"Even after accounting for analyst review time, your net yearly ROI uplift is **₹ {net_savings_monthly*12/10000000:.3f} Cr**!"
    )
...
if not run_sim:
    ...
    kpi_fraud_cr.metric(
        "🛡️ Fraud Prevented", "₹ — Cr", help="Run simulation to populate"
    )
    ...
    training_placeholder.plotly_chart(
        make_training_chart(), use_container_width=True, key="static_training"
    )

    # Show static gauge at 0
    gauge_placeholder.plotly_chart(
        make_gauge(0.0, False), use_container_width=True, key="static_gauge"
    )
    graph_placeholder.plotly_chart(
        make_network_graph(False, 0), use_container_width=True, key="static_net"
    )

    # Static baseline and health
    health_placeholder.plotly_chart(
        make_health_chart(False, 0.45), use_container_width=True, key="static_health"
    )
    baseline_placeholder.plotly_chart(
        make_baseline_chart(), use_container_width=True, key="static_base"
    )
    ...
    for step in range(1, 51):
        ...
        is_attack = step % 7 == 0
        tx_amt = float(
            np.random.randint(40_000, 95_000)
            if is_attack
            else np.random.randint(300, 8_000)
        )

        mock_payload = {
            "TransactionID": str(3_000_000 + step),
        ...
        # Try live A/B API, fall back to local simulation
        try:
            # Query the A/B testing API
            headers = {"X-API-Key": "test-key-123"}
            resp = requests.post(
                API_AB_URL, json=mock_payload, headers=headers, timeout=0.5
            )
            rj = resp.json()
            ab_group = rj.get("ab_group", "A")
            fraud_score = rj.get("fraud_score", 0.5)
            api_latency = rj.get("processing_latency_ms", 38.0)

            # Fetch live A/B status
            status_resp = requests.get(API_STATUS_URL, headers=headers, timeout=0.5)
            status_data = status_resp.json()
            ab_req_a = status_data["group_a"]["total_requests"]
            ab_req_b = status_data["group_b"]["total_requests"]
            ab_fraud_a = status_data["group_a"]["fraud_detected"]
            ab_fraud_b = status_data["group_b"]["fraud_detected"]
            ab_lat_a = [status_data["group_a"]["avg_latency_ms"]] * 5
            ab_lat_b = [status_data["group_b"]["avg_latency_ms"]] * 5
        except Exception:
            # Local A/B simulation fallback
            hasher = hashlib.md5(mock_payload["TransactionID"].encode("utf-8"))
            ab_group = (
                "A" if int(hasher.hexdigest(), 16) % 10000 / 10000.0 < 0.5 else "B"
            )

            if ab_group == "A":
                ab_req_a += 1
                fraud_score = (
                    0.94 if is_attack else float(np.random.uniform(0.01, 0.15))
                )
                api_latency = float(np.random.uniform(28.0, 48.0))
                ab_lat_a.append(api_latency)
                is_fraud_val = fraud_score >= decision_threshold
                if is_fraud_val:
                    ab_fraud_a += 1
            else:
                ab_req_b += 1
                fraud_score = (
                    0.35 if is_attack else float(np.random.uniform(0.01, 0.18))
                )
                api_latency = float(np.random.uniform(2.0, 8.0))
                ab_lat_b.append(api_latency)
                is_fraud_val = fraud_score >= decision_threshold
                if is_fraud_val:
                    ab_fraud_b += 1
        ...
        kpi_roi.metric(
            "📈 Business ROI", f"{int(roi)}x", delta=f"+{total_fraud_caught} catch(es)"
        )
        kpi_recall.metric(
            "🎯 Threshold",
            f"{decision_threshold:.0%}",
            delta=f"score {fraud_score:.2f}",
        )
        kpi_latency.metric("⚡ P99 Latency SLA", f"{p99_lat:.1f} ms")
        kpi_intercepted.metric(
            "🔥 Adversaries Blocked",
            f"{total_fraud_caught}",
            delta=f"-{total_missed_fraud} missed",
        )

        # ── Update KPI cards in Tab 2 ──
        rate_a_pr = (ab_fraud_a / ab_req_a * 100.0) if ab_req_a > 0 else 0.0
        rate_b_pr = (ab_fraud_b / ab_req_b * 100.0) if ab_req_b > 0 else 0.0
        avg_lat_a_val = sum(ab_lat_a) / len(ab_lat_a) if ab_lat_a else 0.0
        avg_lat_b_val = sum(ab_lat_b) / len(ab_lat_b) if ab_lat_b else 0.0

        ab_kpi_a.metric(
            "Group A: GraphSAGE",
            f"{rate_a_pr:.1f}% fraud rate",
            delta=f"{ab_req_a} reqs | {avg_lat_a_val:.1f}ms",
        )
        ab_kpi_b.metric(
            "Group B: XGBoost",
            f"{rate_b_pr:.1f}% fraud rate",
            delta=f"{ab_req_b} reqs | {avg_lat_b_val:.1f}ms",
        )

        z_stats = run_local_z_test(ab_req_a, ab_fraud_a, ab_req_b, ab_fraud_b)
        ab_kpi_z.metric(
            "Z-Test Significance",
            f"p = {z_stats['p_value']:.4f}",
            delta=z_stats["message"],
        )
        ...
        # ── Gauge ──
        gauge_placeholder.plotly_chart(
            make_gauge(fraud_score, is_fraud),
        ...
        # ── Network graph ──
        graph_placeholder.plotly_chart(
            make_network_graph(is_fraud, step),
            use_container_width=True,
            key=f"net_{step}",
        )
        ...
        # ── Tab 2 A/B Charts ──
        fig_ab_lat, fig_ab_rate = make_ab_comparison_charts(
            ab_req_a, ab_fraud_a, ab_req_b, ab_fraud_b, ab_lat_a, ab_lat_b
        )
        ab_chart_lat.plotly_chart(
            fig_ab_lat, use_container_width=True, key=f"ab_lat_{step}"
        )
        ab_chart_rate.plotly_chart(
            fig_ab_rate, use_container_width=True, key=f"ab_rate_{step}"
        )
        ...
        psi_val = (
            np.random.uniform(0.25, 0.8) if drift else np.random.uniform(0.01, 0.15)
        )
        health_placeholder.plotly_chart(
            make_health_chart(drift, psi_val),
            use_container_width=True,
            key=f"health_{step}",
        )
        baseline_placeholder.plotly_chart(
